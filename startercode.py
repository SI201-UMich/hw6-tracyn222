...
def search_breed(breed_id):
    ...
    try:
        ...
        if (
            ...
            and parsed_json["data"] is not None
            and parsed_json["data"].get("id") == breed_id
        ):
            return (parsed_json, response.url)
...
def get_longest_lifespan_breed(cache_file):
    ...
    for breed_info in cache.values():
        data = breed_info.get("data", {})
        attributes = data.get("attributes", {})
        name = attributes.get("name")
        max_life = attributes.get("life", {}).get("max")
...
def recommend_breeds_in_same_group(breed_name, cache_file):
    ...
    for breed_info in cache.values():
        ...
        if isinstance(current_name, str) and current_name.lower() == breed_name.lower():
            target_name_actual = current_name
            target_group_id = (
    ...
    for breed_info in cache.values():
        ...
        if (
            ...
            and current_name.lower() != target_name_actual.lower()
            and current_group_id == target_group_id
        ):
